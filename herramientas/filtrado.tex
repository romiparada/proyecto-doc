...
USeR (User Story eReviewer) & Filtrada, implementación no disponible y academico  & 16/11/2025 \\
...
Ambiguity Detection Using LLMs (ChaseMcClellan) & Filtrada, el archivo sobre generación de historias de usuario esta vacio & 18/11/2025 \\
...
Por lo tanto se tienen 17 herramientas que mencionan en su documentación la generación de historias de usuario. Estas se listan en la Tabla \ref{table:filtrado2}.
...
En primera instancia fue posible descartar todas las herramientas que no se pudieron probar; Beam AI User Stories no fue posible contactarse para conseguir acceso a la plataforma. AI Agile Story/Bug Generation da error al intentar instalarla y AutoStory: AI-Powered Ticket Creation for Jira está desactualizada y no es posible probarla.

Para las herramientas que si se obtuvo acceso se utilizó un documento técnico que contiene requisitos técnicos que permiten generar historias de usuario e incluye una inconsistencia que la herramienta podría detectar. Junto al documento también se utilizó, para las herramientas que lo requieran, un prompt simple que instruye al asistente a generar las historias, incluir criterios de aceptación, referencias al documento y conflictos.
...
De esta manera se comprobaran los requisitos: Generación de Historias con IA, Es automática (no requiere prompt), Inclusión de criterios de aceptación, Detección de inconsistencias, Permite Validar/Refinar las Historias, Trazabilidad.
...
Miro, fue incluida, requiere de prompt y en la prueba realizada generó las historias, incluyendo todos los requerimientos a excepción de la validación/refinado de historias, que no fue encontrada la manera en la primera prueba de la herramienta.
...
AutoDev Editor fue incluida ya que genera historias de usuario. Tiene para elegir varios modelos para la generación pero solo funciona uno, y no se mantiene consistente en la generación cuando se refinan las historias, no funciona bien. Por lo que no se puede refinar historias generadas, tampoco detectó las inconsistencias ni contiene trazabilidad.

Specifai fue incluida ya que genera en base a un título y una mínima descripción opcional, luego permite agregar requisitos en formato texto. Se puede iterar sobre lo generado, segun la vista previa de la aplicacion contiene un chat, pero no fue posible encontrarlo y utilizarlo en la prueba.
...
Storygenie fue incluida, genera los registro con las historias de usuario, solo con un título y una descripción, no incluye criterios de aceptación, trazabilidad y tampoco detecta inconsistencias.

En la Tabla \ref{table:filtrados3}, se muestra el estado de cada herramienta luego de este filtrado.
...
En la tabla \ref{tab:incluidasfiltrado2} se lista para todas las herramientas incluidas junto a los requisitos que cumplen.
...
Por lo tanto tenemos 12 herramientas para evaluar sus historias de usuario. Se decidió hacer un ranking dados los requisitos, para seleccionar las herramientas más completas.
