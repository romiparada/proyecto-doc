...
Una de las plantillas utilizadas para la redacción de historias de usuario es: Como un [tipo de usuario], quiero [hacer algo] para que [se cumpla algún objetivo]. Está compuesta entonces por un rol, una acción y un contexto o propósito.
...
En 2020, el artículo Language Models are Few-Shot Learners~\cite{brown2020language} presentó el modelo GPT-3, que popularizó el uso de la inteligencia artificial en los años siguientes.~\cite{miller2023artificial}

\subsection{Instrucción (Prompt)}

Para utilizar los modelos en tareas de predicción, la entrada original $x$ se modifica mediante una plantilla para convertirla en una cadena de texto (prompt) $x'$ que contiene espacios sin completar. Luego, se utiliza el modelo de lenguaje para completar la información faltante de forma probabilística.~\cite{liu2021prompting}
...
Por ejemplo, en la Figura~\ref{fig:ejemplo_prompt} se muestra un prompt que indica traducir al inglés la frase ``Hola Mundo''.

\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{figs/ejemploprompt.png}
    \caption{Ejemplo de prompt para la traducción al inglés}
